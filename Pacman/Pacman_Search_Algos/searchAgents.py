...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** BEGIN YOUR CODE HERE ***"
        # return (self.startingPosition, self.startingPosition in self.corners)
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [
            ...
            "*** BEGIN YOUR CODE HERE ***"
            dx, dy = Actions.directionToVector(action)
            nextXY = nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty]:
                nextC = list(state[1])

                try:
                    ind = self.corners.index(nextXY)
                    nextC[ind] = True
                except:
                    pass
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    U = [corners[i] for i in range(4) if not visited[i]]
    h = 0
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    if F != []:
        ...
        for food in F:
            costAB = mazeDistance(position, food, problem.startingGameState)
            maxH = max(maxH, costAB)
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        prob = PositionSearchProblem(
            gameState, start=startPosition, goal=closest, warn=False, visualize=False
        )
