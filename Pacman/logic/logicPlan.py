...
def sentence3() -> Expr:
    ...
    PacmanAlive0 = PropSymbolExpr('PacmanAlive', time=0)
    PacmanAlive1= PropSymbolExpr('PacmanAlive', time=1)
    PacmanBorn= PropSymbolExpr('PacmanBorn', time=0)
    PacmanKilled = PropSymbolExpr('PacmanKilled', time=0)

    # ((PacmanAlive_1 <=> ((PacmanAlive_0 & ~PacmanKilled_0) | (~PacmanAlive_0 & PacmanBorn_0))) & ~(PacmanAlive_0 & PacmanBorn_0) & PacmanBorn_0)


    # "Pacman is alive at time 1 if and only if Pacman was alive at time 0 and it was not killed at time 0 or it was not alive at time 0 and it was born at time 0."

    p1 = PacmanAlive1 % ((PacmanAlive0 & ~PacmanKilled) | (~PacmanAlive0 & PacmanBorn))

    # "Pacman cannot both be alive at time 0 and be born at time 0."

    p2 = ~(PacmanAlive0 & PacmanBorn)

    return conjoin([p1, p2, PacmanBorn])
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # KB ⊨ Q equivalent to not SAT(KB ⋀ ¬Q)

    p = premise & ~conclusion

    sat = bool(findModel(p))
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    not a or not b = so a and b can't both be true

    (~A | ~B) & (~A | ~C) & (~B | ~C)  => (0 | 1) & (0 | 1) & (1 & 1)

    At most one can be True to make this expression True. 
    For example, if A is True, then ~A = False. So to make any one of
    the clauses true, B and C must be False => ~B and ~ C are True
    """
    ...
    disjunctions = [disjoin([~a, ~b]) for a, b in combinations]

    return conjoin(disjunctions)
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for (x, y) in all_coords:
        ...
        pacman_xy = PropSymbolExpr(pacman_str, x, y, time=t)

        # a wall at x, y implies it's not the case  pacman is there
        pacphysics_sentences.append( wall_xy >> ~pacman_xy )

    # - Pacman is at exactly one of the squares at timestep t.

    pacman_all_coords = [PropSymbolExpr(pacman_str, x, y, time = t) for (x, y) in non_outer_wall_coords ]
    pacphysics_sentences.append(exactlyOne(pacman_all_coords))

    # - Pacman takes exactly one action at timestep t.
    
    pacman_all_actions = [PropSymbolExpr(d, time = t) for d in DIRECTIONS ]
    pacphysics_sentences.append(exactlyOne(pacman_all_actions))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    KB.extend(
        (
            pacphysicsAxioms(1, all_coords, non_outer_wall_coords, walls_grid, None, allLegalSuccessorAxioms),
            pacphysicsAxioms(0, all_coords, non_outer_wall_coords, walls_grid, None, None),
    ...
    kb = conjoin(KB)
    q = PropSymbolExpr(pacman_str, x1, y1, time=1)

    #entails # KB ⊨ Q equivalent to not SAT(KB ⋀ ¬Q)

    return findModel(kb & q), findModel(kb & ~q)
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        pacman_succs = [pacmanSuccessorAxiomSingle(x, y, t+1, walls_grid) for (x, y) in non_wall_coords]
        pacman_places = [PropSymbolExpr(pacman_str, x, y, time=t) for (x, y) in non_wall_coords]
        moves = [PropSymbolExpr(action, time=t) for action in actions]

        # Add constraints to the knowledge base
        KB.append(exactlyOne(pacman_places))
        KB.append(exactlyOne(moves))
        KB += pacman_succs
        kb = conjoin(KB)

        goal = PropSymbolExpr(pacman_str, xg, yg, time=t)
        goal_str = conjoin(goal, kb)

        if model := findModel(goal_str):
            return extractActionSequence(model, actions)

    # If no solution is found after 50 steps, return None
    return None
...
def foodLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # Initialize KB with initial state
    initial_pacman = PropSymbolExpr(pacman_str, x0, y0, time=0)
    KB = [initial_pacman] + [PropSymbolExpr(food_str, x, y, time=0) for x, y in food]

    for t in range(50):
        print("Time is:", t)

        # Generate propositions for all possible spots
        pacman_places = [PropSymbolExpr(pacman_str, x, y, time=t) for x, y in non_wall_coords]
        all_food = [PropSymbolExpr(food_str, x, y, time=t) for x, y in non_wall_coords]

        # Add constraints to KB
        moves = [PropSymbolExpr(action, time=t) for action in actions]
        KB.extend((exactlyOne(moves), exactlyOne(pacman_places)))

        # Check if goal is satisfied
        kb = conjoin(KB)
        goal = ~disjoin(all_food)
        goal_str = conjoin(goal, kb)
        
        if model := findModel(goal_str):
            return extractActionSequence(model, actions)

        # Generate successor statements for each spot
        pacman_succs = [pacmanSuccessorAxiomSingle(x, y, t+1, walls) for x, y in non_wall_coords]
        KB.append(conjoin(pacman_succs))

        # Add the rule that if food was here and pacman wasn't, then there will be food here next time
        for x, y in non_wall_coords:
            premise_1 = PropSymbolExpr(food_str, x, y, time=t)
            premise_2 = ~PropSymbolExpr(pacman_str, x, y, time=t)
            conclusion = PropSymbolExpr(food_str, x, y, time=t+1)
            KB.append(conclusion % conjoin(premise_1, premise_2))
...
def localization(problem, agent) -> Generator:
    ...
    # First implement setup stuff that holds at all time steps
    # For example, we know where all of the walls are, and where all of the walls are not
    "*** BEGIN YOUR CODE HERE ***"    
    ...
    # to compute `possible_locations` of pacman
    for t in range(agent.num_timesteps):
        # Store a list of (x,y) coordinates
        ...
        KB.extend((
            ...
            PropSymbolExpr(agent.actions[t], t),
            fourBitPerceptRules(t, agent.getPercepts())
        ))

        for (x,y) in non_outer_wall_coords:
            #premise (kb) conclusion (q)
            q = PropSymbolExpr(pacman_str, x, y, time=t) 
            kb = conjoin(KB) 
...
def mapping(problem, agent) -> Generator:
    ...
    # First implement setup stuff that holds at all time steps, or on the initial time step
    # For example, we know the initial location of pacman, and we know that there cannot be a wall at that location
    "*** BEGIN YOUR CODE HERE ***"
    #initialize KB and include a wall isn't where pacman is
    KB.extend((PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time = 0), ~PropSymbolExpr(wall_str, pac_x_0, pac_y_0)))

    "*** END YOUR CODE HERE ***"

    # Loop over every time step and add/solve constraints sufficient
    # to compute `known_map` after `t` time steps
    for t in range(agent.num_timesteps):
        "*** BEGIN YOUR CODE HERE ***"
        # Add pacphysics, action, and percept information to KB
        KB.extend((
        ...
        for (x,y) in non_outer_wall_coords:
            ...
            # update KB and known_map for places where there might be a wall
            if entails(kb, wall_xy):
                KB.append(wall_xy)
                known_map[x][y] = 1
            # and for places where there might not be a wall
            elif entails(kb, ~wall_xy):
                KB.append(~wall_xy)
                known_map[x][y] = 0
...
def slam(problem, agent) -> Generator:
    ...
    # Update known_map to also say there's no wall there
    known_map[pac_x_0][pac_y_0] = 0
    "*** END YOUR CODE HERE ***"

    # Loop over every time step and add/solve constraints sufficient
    # to compute `known_map`, `possible_locations` after `t` time steps
    for t in range(agent.num_timesteps):
        # Store a list of (x,y) coordinates
        possible_locations = []
        ...
        for (x,y) in non_outer_wall_coords:
            ...
            # find provable pacman places
            pac_xy = PropSymbolExpr(pacman_str, x, y, time = t)

            if (findModel(kb & pac_xy)):
                possible_locations.append((x,y))
            elif entails(kb, pac_xy):
                KB.append(pac_xy)
            else:
                KB.append(~pac_xy)
